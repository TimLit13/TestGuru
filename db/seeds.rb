...
user_last_name = user_name.split(' ').last
user_email = Faker::Internet.email
User.create(first_name: user_first_name, last_name: user_last_name, email: user_email, admin: true)

category = Category.create(title: "Ruby on Rails")

test_title = "First test"
test_level = rand(1..5)
test_author = User.first
Test.create!(title: test_title, level: test_level, category_id: category.id, author: test_author)

1.upto(2) do |i|
  question_body = "Question #{i}"
  question_test_id = Test.first.id
  Question.create(body: question_body, test_id: question_test_id)

  1.upto(3) do |j|
    answer_body = "Answer #{j}"
    answer_correct = if j == 1
                      true
                    else
                      false
                    end
    Answer.create(body: answer_body, question_id: Question.find(i).id, correct: answer_correct)
  end
end
